...
本文档专为 **iStore UPS Manager (`luci-app-istore-ups`)** 上架至 **iStoreOS 官方软件中心 (iStore App Store)** 编写，涵盖架构规范、依赖隔离、多 CPU 架构支持、安装与卸载生命周期及 GitHub PR 提交流程。
...
| **2. 依赖隔离与干净卸载** | 卸载应用时绝不可连带卸载其他插件公用的系统基础库 | 在 `app-meta-istore-ups/Makefile` 中，`META_DEPENDS` 仅声明主包 `+luci-app-istore-ups`；通用依赖 (`nut`, `curl` 等) 由底层包管理器自动解析引用计数，杜绝卸载时发生误删。 |
| **3. 完整的生命周期钩子** | 支持一键安装立即运行、干净卸载不留僵死进程 | 主包 `Makefile` 完整实现了：<br>• `postinst`：自动应用初始配置、注册开机自启、启动服务并清理 LuCI 视图缓存。<br>• `prerm`：优雅停止并注销 `istore-ups` procd 守护服务。<br>• `postrm`：自动清理 `/tmp/run` 与 `/tmp/log` 下的临时套接字和缓存。 |
| **4. 软件中心元数据与图标** | 图标 PNG 格式、≤256x256、文件大小 < 50KB | 已在 `applications/app-meta-istore-ups/` 准备 `logo.png`（2.5KB，高清矢量风格）、`Makefile` 与自动配置钩子 `config.sh`。 |
...
```bash
git checkout -b add_app_istore_ups
```
...
3. PR 标题推荐命名：`Add app-meta-istore-ups: iStore UPS 电源管理`。
4. PR 描述中注明：
   - **源码仓库**：`https://github.com/liuyuhao1023/luci-app-istore-ups`
   - **支持架构**：全架构兼容 (`all`)
...
- 验证要点：
  ...
  3. 后台守护进程 `pgrep -f istore-ups-daemon` 正常运行。

### 3. 在目标设备上测试干净卸载
```bash
# 停止并移除插件
/etc/init.d/istore-ups stop
/etc/init.d/istore-ups disable
rm -f /etc/init.d/istore-ups
rm -f /usr/bin/istore-ups-*
rm -f /usr/libexec/rpcd/luci.istore-ups
rm -rf /www/luci-static/resources/view/istore-ups
rm -f /usr/share/luci/menu.d/luci-app-istore-ups.json
rm -f /usr/share/rpcd/acl.d/luci-app-istore-ups.json
